...
func (a *Admin) renderClientsPage(w http.ResponseWriter, r *http.Request, session store.Session, enrollment store.EnrollmentCode, status int, message string, messageIsError bool) {
	...
	if data.Message == "" {
		...
		case "created":
			data.Message = "Spiel und erkannte Version wurden als deaktivierte Katalogentwürfe angelegt. Wähle im Katalog noch Quelle und Quellpfad."
			data.CatalogTab = "game-versions"
		case "linked":
			data.Message = "Inventarfund wurde dem Katalogeintrag zugeordnet."
		case "version-created":
			data.Message = "Die neu erkannte Version wurde als deaktivierter Entwurf angelegt. Wähle im Katalog noch Quelle und Quellpfad."
			data.CatalogTab = "game-versions"
		case "game-created":
			data.Message = "Das Spiel wurde als deaktivierter Katalogentwurf angelegt. Eine Version wurde nicht angelegt; ergänze sie später im Katalog zusammen mit Quelle und Quellpfad."
...
<link rel="stylesheet" href="/admin/assets/management.css?v=20260718-2">
</head>
<body><div class="shell"><aside>
  ...
  {{if .Clients}}<div class="clients-grid">{{range .Clients}}{{$client := .}}
    <article class="card dashboard-card client-card">
      ...
      {{if .Installations}}<div class="table-wrap"><table class="inventory-table"><thead><tr><th>Spiel</th><th>Launcher / ID</th><th>Version</th><th>Installationspfad</th><th>Katalog</th></tr></thead><tbody>{{range .Installations}}{{$item := .}}
