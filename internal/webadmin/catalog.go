  ...
  <section class="card editor" id="catalog-editor" hidden aria-labelledby="editor-title" aria-busy="false" tabindex="-1">
    ...
    <form id="catalog-form" novalidate>
      ...
      <div class="grid">
        ...
        <label class="field" data-kinds="launcher"><span>Adapter</span><select class="select" id="entity-adapter"><option value="steam">Steam</option><option value="ea_app">EA App</option><option value="ubisoft_connect">Ubisoft Connect</option><option value="standalone">Ohne Launcher</option></select></label>

        <label class="field" data-kinds="game"><span>Launcher / Typ <span class="required-mark" aria-hidden="true">*</span></span><select class="select" id="entity-launcher" aria-required="true" aria-describedby="entity-launcher-error"></select><small class="field-error" id="entity-launcher-error"></small></label>
        <label class="field" data-kinds="game"><span>Externe Spiel-ID</span><input class="input" id="entity-external-id" maxlength="256" autocomplete="off"><small class="field-hint">Zum Beispiel Steam App-ID 730. Bei „Ohne Launcher“ verwendet LANReady automatisch den Spiel-Slug.</small></label>
