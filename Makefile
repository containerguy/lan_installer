...
EVENT_RELEASE_PUBLIC_KEY ?= $(shell tr -d '\r\n' < keys/lanready-event-public.key)
AUTHENTICODE_PUBLISHER_SHA256 ?=
CERTIFICATE_THUMBPRINT ?=
TIMESTAMP_URL ?=
POWERSHELL_BIN ?= pwsh
BASE_LDFLAGS := -s -w -X main.version=$(VERSION)
GUI_LDFLAGS := $(BASE_LDFLAGS) -X main.releasePublicKey=$(RELEASE_PUBLIC_KEY) -X main.eventReleasePublicKey=$(EVENT_RELEASE_PUBLIC_KEY) -X main.authenticodePublisherSHA256=$(AUTHENTICODE_PUBLISHER_SHA256)
WAILS_PRODUCT_VERSION := $(shell node -p "require('./cmd/lanready-gui/wails.json').info.productVersion")

.PHONY: all test web-test vet docs-check clean windows windows-gui windows-package check-windows-version check-release-keys linux
...
docs-check:
	...
	python3 -m unittest scripts.verify_docs_test scripts.lanready_secrets_test scripts.lanready_compose_paths_test scripts.validate_release_keys_test
...
check-release-keys:
	python3 scripts/validate_release_keys.py "$(RELEASE_PUBLIC_KEY)" "$(EVENT_RELEASE_PUBLIC_KEY)"

windows-gui: check-windows-version check-release-keys
...
windows-package: check-windows-version check-release-keys
	...
	$(POWERSHELL_BIN) -NoProfile -File scripts/build-windows-release.ps1 -Version "$(VERSION)" -ReleasePublicKey "$(RELEASE_PUBLIC_KEY)" -EventReleasePublicKey "$(EVENT_RELEASE_PUBLIC_KEY)" -PublisherCertificateSHA256 "$(AUTHENTICODE_PUBLISHER_SHA256)" -CertificateThumbprint "$(CERTIFICATE_THUMBPRINT)" -TimestampUrl "$(TIMESTAMP_URL)"
