...
- Noch nicht als MVP abgenommen: reale Windows-11-Tests der drei Launcheradapter und des Task-Scheduler-/Toast-Ablaufs, Installations-/Updateorchestrierung für Launcher/Spiele sowie ein Ende-zu-Ende-Self-Update mit öffentlich vertrauenswürdig Authenticode-signiertem und zeitgestempeltem Release. Das produktive Serverbinary läuft auf `90e8b64` und enthält die gehärtete manuelle Cache-Garbage-Collection mit Management-UI; `LANREADY_VERSION` wurde passend aktualisiert. Vollständige Tests, Vet, Race-Detector für Store/Artifact/Webadmin und unabhängiger P0/P1-Review sind grün. Nach dem Deployment wurden laufende Version, Container-Health, read-only Root-Filesystem, NPM-Netz, NFS-Mount und der öffentliche HTTPS-Healthcheck geprüft. Die destruktive GC-Mutation wurde produktiv bewusst nicht gegen reale Daten ausgelöst; API, Idempotenz, Partial-Result, Rollen und Race-Semantik sind durch Integrationstests abgedeckt. `LANREADY_AUTHENTICODE_PUBLISHER_SHA256` bleibt bis zu einem echten Codesigning-Zertifikat absichtlich leer, sodass Clientupdates fail-closed bleiben.
