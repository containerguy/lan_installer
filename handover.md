...
- Paket-/Standalone-Aktionen sind zusätzlich zentral in Publish und Activate gesperrt; Aktivierung ab `0.2.0` verlangt ein gültiges Stable-Clientupdate und passende Laufzeitversionen aller aktiven PCs. Update- und Event-Keyrings sind zweckgebunden. Der alte Offline-Public-Key bleibt nur als Legacy-Event-Verifikationskey erhalten; Rollbacks werden mit dem Online-Event-Key und Mindestclient `0.2.0` neu signiert. Build, Serverstart und Backup lehnen identische Online-/Offline-Keys ab.

## Produktiv ausgerollt – Browserbasierte Event-Releases (21.07.2026)

- Deployment `81de128`. Der netzwerklose `signer-service`-Container hält als einziger den Online-Event-Private-Key und ist ausschließlich über den Unix-Socket `data/signer/signer.sock` erreichbar. Produktiv geprüft: Socket-Modus `0600`, `network_mode=none`, `read_only`, `cap_drop: ALL`, Startreihenfolge über `depends_on: service_healthy`. Im Servercontainer liegen nur Public Keys, der Socketpfad ist read-only gemountet; im Signer-Container liegt nur der Private Key.
- Produktiver Online-Event-Key: Key-ID `ed25519-5512acafe478acd5`. `scripts/validate_release_keys.py` bestätigt die Verschiedenheit vom Offline-Update-Key. Das Secret-Backup erfasst jetzt vier Dateien einschließlich des Event-Public-Keys; der Private Key bleibt bewusst ausgeschlossen.
- **Offener Betriebspunkt:** Die unverschlüsselte Kopie des Event-Private-Keys unter `/home/ubuntu/lanready/backups/event-key-20260721T122128Z/` muss noch verschlüsselt und getrennt vom Server abgelegt werden; danach ist über das Löschen der Klartextkopie zu entscheiden.
- Authentifizierte Ende-zu-Ende-Prüfung über Nginx Proxy Manager/TLS: Login, `/admin/events`, `/admin/catalog`, `/admin/clients` und `/admin/api/v1/releases/status` je HTTP 200. Der Preflight für `markuslan-20260724` liefert erwartungsgemäß `ready: false` mit genau zwei `client_installation_missing`-Blockern (FlatOut 2, WC3 TFT). Es wurde bewusst kein Release erzeugt, signiert oder aktiviert.
- Container healthy, SQLite `quick_check=ok` und null Foreign-Key-Verletzungen, öffentlicher HTTPS-Healthcheck grün.
- Rollback: `/home/ubuntu/lanready/backups/pre-81de128-20260721T120528Z` sowie Image `lanready-server:rollback-pre-81de128`. Weil dieses Deployment die Compose-Topologie erweitert, muss ein Rollback zusätzlich die gesicherte `compose.yaml` einspielen und den `signer-service` stoppen und entfernen; ein reiner Image-Tausch genügt nicht.
- Der Windows-GUI-Build steht weiterhin aus. Das blockiert den Serverstand nicht: Ohne kompatibles signiertes Stable-Clientupdate und passende Laufzeitversionen aller aktiven PCs lässt sich serverseitig kein Event aktivieren.
