...
- Der NFS-Cachepfad `192.168.200.200:/mnt/S01/Shares/GameManager` ist auf TrueNAS sicher für die Docker-VM vorbereitet, aber noch nicht dauerhaft auf der VM gemountet oder in Compose aktiviert. Verifiziert sind TCP 2049, `showmount`, NFSv3/v4, ein NFSv4.2-Hard-Mount und rund 1,7 TiB freier Platz. TrueNAS besitzt den gesperrten, passwortlosen Nicht-SMB-Account `lanready-nfs` (UID/GID 3001); NFS-Share 1 ist ausschließlich auf Host `192.168.220.39` begrenzt, hat keine Netzfreigaben, ist RW, mappt User und Gruppe vollständig auf `lanready-nfs` und verwendet `SYS`. Das leere, ausschließlich per NFS veröffentlichte Dataset nutzt jetzt POSIX1e mit `aclmode=DISCARD`: Root-Owner `rwx`, UID 3000 (`lan`) Access+Default `r-x`, UID 3001 Access+Default `rwx`, Group/Other ohne Rechte und Mask `rwx`. Der vorherige NFSv4-Ansatz wurde verworfen, weil TrueNAS trotz expliziter DENY-ACE beliebiges `chown` durch den gemappten Eigentümer erlaubte. Der explizit als Host-UID/GID 1000 laufende Abnahmetest ist für Create/Write/Fsync/Read/Rename/Delete grün; neue Objekte gehören serverseitig 3001:3001, `chown` auf 0:0 und 3000:3000 wird verweigert. Ein separater Test ohne `mapall` bestätigt für UID 3000 List/Traverse/Read und verweigert Create/Write/Truncate/Rename/Delete; TrueNAS-Readback belegt `r-x` auf dem Child-Verzeichnis und effektiv `r--` auf der Datei bei `other---`. Als POSIX-Dateieigentümer darf der Cacheprozess weiterhin die Modusbits eigener Cachedateien ändern; da der hostgebundene Writer ohnehin alle Cacheinhalte ersetzen und löschen kann, ist das ein dokumentiertes P2-Restrisiko und keine zusätzliche Sicherheitsgrenze. API-Readback bestätigt exakt einen überlappenden NFS-Share und keine SMB-/WebDAV-Freigabe. Probeobjekte und temporäre Mounts sind entfernt, das Dataset ist leer. Schlüsselbereinigte Prestate-Backups liegen mit Eigentümer `ubuntu:ubuntu` und Modus 0600 unter `/home/ubuntu/lanready/backups/truenas-nfs-*/`; eine Markersuche fand keine Authorization-, Bearer- oder API-Key-Felder. Compose und `/cache` bleiben bis zum nächsten Slice unverändert auf dem lokalen Fallback.
