...
- Backup/Restore sichert SQLite konsistent, löst alle drei Secret-Quelldateien aus der effektiven Compose-Konfiguration auf, archiviert das alte Serverimage per `docker image save` und behandelt externen CAS, Konfiguration und Datenbank als gemeinsamen Stand. Restore stoppt fail-closed, arbeitet mit einem vollständig geprüften Staging-Datenbaum, erhält alte Daten/Secrets/Konfiguration, mischt keine Verzeichnisstände und startet erst nach Image-, Mount-, Sentinel- und Schreibprüfung.
...
- `make test` enthält nun `docs-check`. Acht Python-Tests decken fehlende Links/Anker, Pfadausbruch, abweichende externe Cachepfade, Cache innerhalb von `data`, drei externe Secretpfade, geänderte Restoreziele und den Fehlerfall eines fehlenden Backup-Secrets ab. Dokumentationsprüfung, Python-Tests, alle Node-Tests, Compose-Konfigurationsprüfung und `git diff --check` sind grün. Der anschließende Go-Schritt war in der aktuellen WSL-Sitzung mangels ausführbarem `go` im PATH nicht erneut möglich; Anwendungs-Go-Code wurde in diesem Slice nicht geändert.
...
- Beim anschließenden Signierablauf-Abgleich wurde ein separater offener P1 bestätigt: Die UI erzeugt noch keinen initialen Event-Payload-Kandidaten. Zusätzlich verlangt das v2-Release-Schema pro Spiel mindestens ein Artefakt-Payload, obwohl der Katalog Provider-only-Versionen ohne LANReady-Paket erlaubt. Diese Versionen können daher noch nicht sinnvoll signiert veröffentlicht werden. Dummy-Artefakte sind ausdrücklich keine zulässige Umgehung; Kandidatengenerator und Releasevertrag bilden den nächsten zusammengehörigen Slice.
