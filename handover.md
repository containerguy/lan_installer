...
- Der geplante NFS-Cache-Mount `192.168.200.200:/mnt/S01/Shares/GameManager` ist noch nicht eingerichtet. Vom Ubuntu-Dockerhost `192.168.220.39` läuft TCP 2049 auch nach der gemeldeten Firewall-Anpassung weiterhin in einen Timeout (`Resource temporarily unavailable`); deshalb wurden weder `nfs-common` installiert noch Compose oder `/cache` umgestellt.
