...
Die Sofortaktivierung ist standardmäßig aus. Vor jeder Aktivierung muss jeder aktive PC die Release-Mindestversion bereits als Laufzeitversion gemeldet haben; ein älterer Client blockiert die Auslieferung mit PC-Name und Versionsstand.

Solange ein PC diese Version noch nicht erreicht hat, verlangt der Server zusätzlich ein im Stable-Kanal veröffentlichtes, mit dem getrennten Offline-Key signiertes Clientupdate auf mindestens diese Version, damit der betroffene PC überhaupt aktualisiert werden kann. Sind bereits alle aktiven PCs auf der Mindestversion, entfällt diese Anforderung: Dann gibt es niemanden mehr zu aktualisieren, und ein manuell verteilter Client kann ohne Self-Update-Paket im CAS aktiviert werden. Neu hinzukommende ältere Clients bleiben dadurch nicht ungeschützt, weil die Geräteendpunkte weiterhin kein Release unterhalb ihrer Mindestversion ausliefern.

Nach dem Client-Rollout kann die bereits signierte Sequenz direkt aus der Historie aktiviert werden. Auch browser-signierte Rollbacks verlangen wegen des neuen Event-Keyrings mindestens Client `0.2.0`.
