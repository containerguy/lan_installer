...
`GET /v2/device/standalone-games` erfordert Geräteauthentisierung und liefert ausschließlich aktive Katalogspiele des aktiven, fest vorgegebenen Adapters `standalone` („Ohne Launcher“). Die stabile externe ID entspricht serverseitig immer dem Spiel-Slug. Freie, vom Client erfundene Katalogeinträge sind ausgeschlossen.

`POST /v2/device/inventory-scans` erfordert Geräteauthentisierung sowie das gebundene Benutzer-Zugriffstoken als `Authorization: Bearer <token>`. Ein Scan besitzt `scanId`, `scannedAt`, `clientVersion` und eine vollständige Liste gefundener Installationen mit `launcher`, `externalGameId`, `displayName`, `detectedVersion`, `versionSource` und `installPath`. Zulässige Launcher sind `steam`, `ea_app`, `ubisoft_connect` und `standalone`. Ein `standalone`-Fund stammt aus einer kataloggebundenen, im geschützten Windows-Benutzerprofil gespeicherten Registrierung. Unbekannte Versionen werden als `detectedVersion: null` mit begründetem `versionSource` übertragen; für manuelle Spiele darf nur die Windows-Dateiversion der gewählten EXE als erkannte Version verwendet werden. Der Client erfindet keine Version. Pfade sind nur für berechtigte Managementbenutzer sichtbar und werden nie in Logs oder Auditdetails geschrieben.
