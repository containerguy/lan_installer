...
Der derzeit ausgerollte Managementserver ist noch kein nach diesem Vertrag abgeschlossenes MVP. Lokal sind der vollständige Quellenablauf, persistente HTTPS/WebDAV-Cache-Aufträge mit Managementstatus und harter Quota, Geräte-Enrollment, Browser-Einmalcode-Anmeldung, Spieleerkennung, bestätigte Inventarsynchronisation, Admin-Katalogimport, eine gestaltete native Windows-GUI, der verifizierende SHA-256-CAS-Kern mit authentifizierten Range-Downloads, ein per-user Agentmodus sowie Veröffentlichung und atomare Installation signierter Clientupdates mit Anti-Rollback und Frontend-Health-Wiederherstellung umgesetzt. Portable ZIP und per-user Installer werden reproduzierbar gebaut, sind aber noch nicht Authenticode-signiert. Es fehlen insbesondere Cache-Garbage-Collection und der reale Nextcloud/NAS-Test, der automatisch aus Katalog/CAS erzeugte Event-Kandidat, die GUI-Installations-/Updateorchestrierung für Spiele und Launcher sowie reale Windows-11-Adapter-, Task-Scheduler-, Benachrichtigungs- und Self-Update-Tests mit einem öffentlich vertrauenswürdigen, zeitgestempelten Build.
